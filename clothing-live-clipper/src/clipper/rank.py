...
_LIVE_ROOM_MARKERS = (
    ...
    # 催手速 / 开架预告 / 抱一下寒暄（强制去直播感）
    "手速", "手要快", "开架", "准备开架", "马上开架", "抱一下", "抱一抱",
    "给大家抱一下", "大家抱一下", "吃饭给大家", "稍等一下", "等我一下",
    # 人设/标签灌鸡汤
...
def _looks_like_live_room(text: str) -> bool:
    ...
    if re.search(r"(扣|点|刷).{0,2}(1|一|关注)", t):
        return True
    # 手速 / 开架 / 抱一下 — 即使夹杂少量服装词也倾向剔除纯过门
    if re.search(r"手\s*速", t) or re.search(r"开\s*架", t) or re.search(r"抱\s*(一\s*下|一\s*抱)", t):
        if not any(h in t for h in ("版型", "显瘦", "面料", "不透", "收腰", "遮肉")):
            return True
